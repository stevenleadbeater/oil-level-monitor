...
pub async fn get_by_distance_id(distance_id: i32) -> Result<Vec<Row>, String> {
...
pub async fn get_latest_distance_history(distance_id: i32) -> Result<Vec<Row>, String> {
    match tokio_postgres::connect(connection_string().clone().as_str(), NoTls).await {
        Ok((client, connection)) => {
            tokio::spawn(async move {
                if let Err(e) = connection.await {
...
pub async fn insert(distance: DistanceHistory) -> Result<(), String> {
    match tokio_postgres::connect(connection_string().clone().as_str(), NoTls).await {
        Ok((client, connection)) => {
            tokio::spawn(async move {
                if let Err(e) = connection.await {
